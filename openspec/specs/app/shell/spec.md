...
SHALL 以 orpc 契约（`src/shared/rpc-contract.ts`，前后端共享类型）暴露 M1 引擎能力的读写面：提供方（services/groups/keys/share/status/serve 控制）、使用方（import/join/key add/ports/status/forget/run 控制）、预设（list/detail/apply）、写手（preview/apply）、系统（主题偏好/设置）。全部变更操作 SHALL 走引擎同款校验（复用 M1 模块，不另写逻辑）。错误 SHALL 经统一边界映射（DomainError→ORPCError），消息英文 ASCII。
