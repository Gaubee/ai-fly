...
- **模式切换**（Owner 2026-09-15 双模式裁决；**同表单复用 Owner 2026-09-16**）：
  「自定义」与「预设」互斥切换，两模式渲染**同一套四阶段纵向管线**（编号 +
  左侧连接线；①②默认展开，③④折叠收纳）——预设模式 = 管线上方多一个整段
  脚本选择器（仅列**至少导出一个阶段函数**的脚本），且四阶段全部 readonly
  （同款控件 disabled + 值硬编码自所选脚本 stages 矩阵；未覆盖阶段显示缺省
  语义）；自定义模式 = 下列逐槽编辑器。不另写第二套呈现样式。切换清空另一侧；
  两模式同现不可能（提交层互斥，见 provider 条款）。
...
- **THEN** 保存后服务为 `hooks: {script: "codex"}`；编辑回显仍为预设模式，且
  四阶段管线呈 readonly 态——①②③阶段槽 disabled 显示 codex、④显示缺省语义，
  与 stages 矩阵一致
