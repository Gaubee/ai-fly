...
status/contentType/正文按 `upstream_status` 语义原样透传。客户端断开时网关
SHALL 发 ABORT（WS 场景发 CLOSE）帧并清理在途状态；本地客户端消费过慢致接收
缓冲达限（默认 4 MiB，WS 双向各自计）时 SHALL 以 `buffer_overflow` 语义终结该
请求并关闭本地连接（内存有界，其它请求不受影响）。
