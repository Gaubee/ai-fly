...
签发时展示原文一次）。密钥原文持久化于本地密钥库（**Owner 裁决 2026-09-13**：
本机可随时取回，取代早期「仅存哈希、签发后不可再现」语义；数据目录 0700、
密钥库文件 0600，本地信任边界即回环边界）；**远程面 MUST NOT 出现原文**——
面板/RPC 列表、目录 detail 投影、usage 日志仅含 keyId/名称/分组/创建时间/状态。
存储同时保留加盐哈希用于校验：密钥校验 MUST 常数时间比较
（SHA-256(salt+原文) 后 timingSafeEqual 扫描，不因命中位置泄露时序）。一分组
可持多枚密钥；密钥可单独撤销（撤销即刻生效：剔除其授权视图或断开无余钥会话，
后续 AUTH 计入 rejected）。密钥与设备无关：任何已入网设备持钥即得访问，撤钥
切断所有持钥设备，撤令牌只踢一台设备（两级撤销）。
...
- **WHEN** 提供方在面板或 RPC 列出密钥、或目录同步推送 detail
- **THEN** 仅出现 keyId、名称、分组、创建时间与状态；原文仅存于本地密钥库（0600），不跨任何 RPC、目录投影或日志
