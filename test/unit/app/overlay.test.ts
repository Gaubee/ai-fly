// 窗口安全区单测（m3 Lane SHELL 2.3；2026-09-10 Owner 裁决修订：left/right 只描述
// 标题带矩形，消费面是 header 的 padding-inline）：测 overlay-geometry.ts 的纯计算
// 与依赖注入编排（反应式薄壳 overlay.svelte.ts 仅做 $state 接线，由
...
//   写 --ot-inset-top / --ot-inset-left / --ot-inset-right；
...
const ZERO: OverlayInsets = { top: 0, left: 0, right: 0 };

/** 宿主替身：记录状态写入 + 以 Map 模拟 CSSStyleDeclaration.setProperty。 */
function makeHost(viewportWidth = 1000): {
  host: OverlayHost;
...
function makeFakeWindow(options: {
  ...
  let released = 0;
  const rect = options.rect ?? { x: 78, y: 28, width: 922, height: 24 };
...
describe("insetsFromRect（度量纯函数）", () => {
  it("标准 macOS 形态：y 即标题带高度、x 即红绿灯右缘；rect 至视口右缘 → right 0", () => {
    expect(insetsFromRect({ x: 77.6, y: 28.4, width: 922.4, height: 24 }, 1000)).toEqual({
      top: 28,
      left: 78,
      right: 0,
    });
  ...
  it("Windows 右置 caption 形态：x=0、rect 右缘止于 caption 左 → right 为 caption 宽", () => {
    expect(insetsFromRect({ x: 0, y: 32, width: 940, height: 24 }, 1000)).toEqual({
      top: 32,
      left: 0,
      right: 60,
    });
  ...
  it("y 度量为 0 时回退保守标题带高度；负值钳 0", () => {
    expect(insetsFromRect({ x: 78, y: 0, width: 922, height: 0 }, 1000).top).toBe(FALLBACK_INSET_TOP);
    expect(insetsFromRect({ x: -5, y: -3, width: 1200, height: 0 }, 1000)).toEqual({
...
describe("attachOverlay（探测 + 订阅编排）", () => {
  it("bridge 缺席（纯浏览器 dev）→ inset 全 0、dragEnabled=false、CSS 变量归零", async () => {
    ...
    expect(last()).toEqual({ insets: ZERO, dragEnabled: false });
    expect(vars.get(INSET_TOP_VAR)).toBe("0px");
    expect(vars.get(INSET_LEFT_VAR)).toBe("0px");
    expect(vars.get(INSET_RIGHT_VAR)).toBe("0px");
    detach();
  });
  ...
  it("overlay 可见：rect {x:78,y:28} → top 28 / left 78，CSS 变量写入根元素", async () => {
    const { host, vars, last } = makeHost(1000);
    const fake = makeFakeWindow({ visible: true, rect: { x: 78, y: 28, width: 922, height: 24 } });
    await attachOverlay(fake.win, host);
    expect(last()).toEqual({ insets: { top: 28, left: 78, right: 0 }, dragEnabled: true });
  ...
  it("overlay 不可见（visible=false）→ 归零且 dragEnabled=false", async () => {
    const { host, last } = makeHost();
    const fake = makeFakeWindow({ visible: false, rect: { x: 78, y: 28, width: 922, height: 24 } });
    await attachOverlay(fake.win, host);
    expect(last()).toEqual({ insets: ZERO, dragEnabled: false });
  });

  it("geometrychange（窗口 resize）→ 以事件载荷即时重算（含右置 caption）", async () => {
    const { host, vars, last } = makeHost(500);
    const fake = makeFakeWindow({ rect: { x: 78, y: 28, width: 422, height: 24 } });
    await attachOverlay(fake.win, host);
    fake.fire({ x: 0, y: 32.4, width: 440, height: 24 });
    expect(last()).toEqual({ insets: { top: 32, left: 0, right: 60 }, dragEnabled: true });
    expect(vars.get(INSET_TOP_VAR)).toBe("32px");
    expect(vars.get(INSET_LEFT_VAR)).toBe("0px");
    expect(vars.get(INSET_RIGHT_VAR)).toBe("60px");
  });
