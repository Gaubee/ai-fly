// secret CLI（cli-hardening #11）：set --value/--stdin/--no-bearer、list 形状、
// remove；值永不出库（list 无值列）。stdout 间谍 + tmp HOME + 真实 SecretsStore。
...
describe("ai-fly secret", () => {
  it("set --value 默认 bearer 前缀；--no-bearer 关闭", async () => {
    const home = freshHome();
    const data = join(home, "provider");
    expect(await run(["set", "k1", "--value", "sk-abc", "--data", data], { homedir: home })).toBe(0);
    expect(await run(["set", "k2", "--value", "sk-def", "--no-bearer", "--data", data], { homedir: home })).toBe(0);
    const store = SecretsStore.open(data);
    expect(store.resolve("k1")?.headerValue).toBe("Bearer sk-abc");
    expect(store.resolve("k2")?.headerValue).toBe("sk-def");
  });
  ...
  it("set --stdin 读一行 stdin（去换行）", async () => {
    ...
    expect(SecretsStore.open(data).resolve("ks")?.headerValue).toBe("Bearer sk-from-stdin");
  });

  it("list：名字/时间/bearer 列，无值列（值不出库）；remove 删除", async () => {
    const home = freshHome();
    const data = join(home, "provider");
    ...
    expect(text).toContain("bearer:on");
